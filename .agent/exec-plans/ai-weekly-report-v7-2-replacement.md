...
- [x] (2026-07-31 16:34+09:00) **Backend Completion Bundle:** 263 tests, Issue #4 failure 1, Errors 0, Skipped 0, repair commit 9f025f4.
- [x] (2026-07-31 16:55+09:00) **M8 완료.** REST API v7-2 계약 교체 (`POST/GET/PDF`), `AiWeeklyReportApiTest` 11건 통과. 커밋 `0c912e4`.
- [x] (2026-07-31 16:56+09:00) **M9 완료.** `OpenHtmlReportPdfRenderer` 4페이지 PDF 렌더링 구현, `OpenHtmlReportPdfRendererV72Test` 3건 통과. 커밋 `fa4838d`.
...
- [x] (2026-07-31 16:58+09:00) **M11 완료.** legacy weekly AI report 클래스/테스트 제거. 커밋 `7f30ee9`.
- [x] (2026-07-31 17:55+09:00) M11 이후 최종 검증 구간에서 결함 4건을 고쳤다.
  커밋 `e8d1199`, `71bd899`, `d473f3c`, `7e78d58`. 이 구간은 당시 Progress에
  기록되지 못한 채 중단됐고 아래 항목으로 소급 정리한다.
- [x] (2026-07-31 18:36+09:00) `e5fcd41`·`7338174`를 사용자 요청으로 revert
  (`7cf3f9c`, `d5373b4`)한 뒤 `f86f377`로 필요한 부분만 복원했다.
- [x] (2026-07-31 20:50+09:00) **기간 제약 완화.** 커밋 `d1e575e`.
  대시보드 주차 선택이 달 기준 날짜 묶음이라 월요일 7일 제약과 구조적으로
  맞지 않았다. 서버 제약을 "이미 끝난 `[from, toExclusive)`"로만 좁히고
  직전 비교 기간을 선택 기간과 같은 길이로 일반화했다. 명세 D3·§4.3 개정.
- [x] (2026-07-31 20:52+09:00) **OpenAI 경로 복구.** 커밋 `a921252`.
  모든 리포트가 `SERVER_FALLBACK`으로 떨어지던 원인 두 가지를 제거했다.
  ① 모델이 되돌려준 `schemaVersion`을 신뢰하던 매퍼 ② validator 규칙이 없던
  프롬프트. fallback 사유 로깅을 추가했고, `.gitignore`의 `*.txt`에 걸려
  **추적된 적이 없던 프롬프트 2개를 force-add** 했다.
- [x] (2026-07-31 20:55+09:00) **산출물 방식 전환.** 커밋 `0c7d395`.
  서버 PDF 대신 인쇄용 HTML을 내려준다(`AiWeeklyReportDocumentService`,
  `GET /{reportId}/download`). 문서·fallback 다국어화, 유료 팀장 전용 노출,
  언어 선택 모달과 재생성 확인 모달을 함께 넣었다.

### 배포 판정 (2026-07-31 21:00+09:00)

실제 OpenAI 호출로 브라우저 E2E를 돌렸다. `analysis_mode=OPENAI`,
`model=gpt-5.6-luna`, 생성 7.4초, 저장본 재사용 49ms, 콘솔 에러 0.
...
### 2026-08-01~02 마무리 구간

배포 전 점검에서 나온 결함을 잡고 문서를 코드에 맞췄다. 커밋 순서대로:

- `1233d34` fallback 문장이 치환과 겹쳐 "위험 후보 해당 위험 후보"가 되던 회귀.
  앞 커밋이 만든 것이고, 검증을 OPENAI 경로로만 해서 놓쳤다.
- `7dbebc6` `downloadUrl`이 옛 `/pdf`를 가리키던 것. 명세 D7·§9.3도 함께 개정.
- `0eaa253` 문서에 남던 내부 코드(`OVERDUE`, `NEXT_REVIEW_DATE_SET` 등) 라벨화.
  기존 사전이 계약에 없는 이름을 담고 있어 실제 값을 하나도 못 맞추고 있었다.
- `5a4a4e9` fallback 성과가 완료 업무를 1건만 싣던 것. 계약은 5건까지 허용한다.
- `6c823a0` 집계가 100건으로 잘린 배열을 쓰던 것. 직전 기간은 잘리지 않아 증감이
  서로 다른 모수로 계산됐다. 100건 넘는 팀에서만 드러난다.
- `bf7a6da` 일정표가 말없이 3건만 싣던 것. 업무표는 밝히는데 규칙이 달랐다.
- `f643f37` revision 번호 충돌이 유료 호출 뒤 500이 되던 것. 재시도와 409로 바꾸고
  `DataIntegrityViolationException` 핸들러를 전역에 추가했다.
- `30a7092` `DemoReadOnlyFilter`가 무력화된 채 커밋돼 있던 것. 공개 데모에서 데이터
  변경이 가능한 상태였다. `AuthSecurityApiTest` 실패 1건의 진짜 원인이었다.
- `865e3ce` 프롬프트 `.txt`가 CI `repository-safety`에 걸려 이 기능이 한 번도 CI를
  통과한 적이 없었다. `.prompt`로 옮겼다.
- `88f6e5d` 문서가 기간과 무관하게 스스로를 "주간"이라 부르던 것.
- `f22d392` AI가 본 업무가 기간 전체보다 적을 때 그 사실을 감추던 것.
- `7da8e10` 주차·기간 select가 모순될 수 있고 그때 코드가 사용자 선택 하나를 버리던 것.
  기본 리포트도 같은 함수를 써서 함께 고쳐졌다.
- `6d40f9d` 명세 §7이 요구하는데 없던 검증(deadline source, 위험 주장 교차 검증).
- `ea8a10a` OpenAI 호출 동안 DB 커넥션을 잡고 있던 것(명세 §14 위반).
- `cde4f58` `6acf525` 명세를 코드에 맞춤. §4 입력 계약 예시가 통째로 이전 세대였다.

## 이 구간에서 배운 것

**검증 경로를 한쪽만 타면 반대쪽에서 회귀가 난다.** ref 치환을 OPENAI 경로로만 확인해
fallback 경로를 깨뜨렸다. 이후 fallback 투영 테스트를 고정했다.

**문서가 여러 세대로 쌓이면 어디부터 읽느냐로 답이 달라진다.** §9.1은 "기간 7일",
D3는 "월간·연간 허용"이 같은 문서에 공존했다. 정본을 schema로 두고 산문을 맞췄다.

**"기존 실패 baseline"으로 분류한 테스트가 실제 결함이었다.** 원인을 안 보고 넘긴 탓에
공개 데모의 쓰기 차단이 꺼진 상태로 하루를 보냈다.

### 다음 세션 인계 (2026-08-02 저녁)

**바로 이어서 할 것 — fixture 4 실패 원인.**

`scripts/ai-report-fixture-matrix.sql`로 상황별 그룹 6개(id 17~22)를 만들었다. 같은 주간
`2026-07-20 ~ 2026-07-27`으로 넷을 생성했고 그중 하나가 실패했다.

    group 17 위험 없음   OPENAI          in 2,429 / out 427
    group 18 지연 다수   OPENAI          in 2,777 / out 1,429
    group 19 승인 대기   OPENAI          in 2,809 / out 1,195
    group 20 부하 편중   SERVER_FALLBACK  gateway 예외
    group 21 대량 105건  미생성
    group 22 업무 없음   미생성

실패는 검증 거부가 아니라 gateway 단계다. 로그에 `IllegalArgumentException`만 남아 SDK
