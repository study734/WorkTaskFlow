...
import { useTaskPasteImport } from '../../task/components/useTaskPasteImport';
...
export function GroupDashboardPage() {
  ...
  const pasteNotice = useTaskPasteImport({ active: showCreate, title, setTitle, checklistItems, setChecklistItems, disabled: saving });
  ...
  return <><AppNavigation /><main className="group-dashboard-page app-page">
    ...
    {showCreate && <Modal title={t('새 업무 만들기', 'Create a task')} description={t(`${dashboard?.groupName ?? '그룹'} 대시보드에서 바로 업무를 추가합니다.`, `Add a task directly from the ${dashboard?.groupName ?? 'group'} dashboard.`)} onClose={() => !saving && setShowCreate(false)}><form className="form modal-form" onSubmit={createTask}>
      <label className="field"><span>{t('제목', 'Title')}</span><input autoFocus required maxLength={120} data-task-paste="title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t('예: 발표 자료 초안 작성', 'e.g. Draft presentation slides')} /><small className="field-help">{t('여러 줄을 붙여넣으면 첫 줄은 제목, 나머지 줄은 체크리스트로 들어갑니다.', 'Paste multiple lines to fill the title from the first line and the checklist from the rest.')}</small></label>
