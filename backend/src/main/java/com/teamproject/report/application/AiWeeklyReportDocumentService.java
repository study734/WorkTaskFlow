...
public class AiWeeklyReportDocumentService {
    ...
    private String pageFour(Doc doc) {
        ...
        for (IssueView issue : issues) {
            ...
            cards.append("<article class=\"decision-card\"><div class=\"decision-top\">")
                    ...
                    .append(cell(doc.ko ? "판단 근거" : "Evidence codes", joinCodes(issue.evidenceCodes())))
    ...
    private String completion(Doc doc, DecisionView decision) {
        ...
        for (String code : decision.completionSignalCodes()) {
            parts.add(switch (code) {
                ...
                case "DUE_SET" -> doc.ko ? "새 마감 저장" : "new due date saved";
                case "STATUS_CHANGED" -> doc.ko ? "상태 변경 기록" : "status change recorded";
    ...
    private String joinCodes(List<String> codes) {
        return codes == null || codes.isEmpty() ? "-" : String.join(", ", codes);
    }
