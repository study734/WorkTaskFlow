...
같은 텍스트 필드에서 **계약 코드와 내부 필드 이름도 사용자 언어로 바꾼다**. 모델은
프롬프트에서 본 어휘를 문장에 옮겨 적는다.

```text
입력  다수에서 OVERDUE 신호가 확인되어 ... 선택할 riskCandidates가 없습니다.
출력  다수에서 마감 초과 신호가 확인되어 ... 선택할 위험 후보가 없습니다.
```

사전은 `AiWeeklyReportCodeVocabulary` 하나다. 문서 렌더러의 코드 라벨도 같은 사전을
쓰므로 화면·문서·산문이 같은 말을 쓴다. 치환 뒤에는 한국어 조사를 앞말의 받침에 맞춰
다시 고른다(`완료율가` → `완료율이`). 프롬프트에도 같은 규칙을 넣지만, 프롬프트는
모델이 지킬 수도 안 지킬 수도 있으므로 서버 치환이 실제 보증이다.
