...
- `AI 리포트` 행동은 현재 페이지를 이동시키지 않는다. 기본 리포트의 `한국어 다운로드`와 같이
  별도 창을 열어 생성 상태를 표시하고, 완료되면 그 창에 리포트와 `PDF 다운로드` 버튼을 그린다.
  저장 리포트가 없으면 같은 행동에서 생성한다.
- 이 행동은 팀장에게 검토 대기 상태를 두지 않는다. 새로 만든 revision은 `DRAFT`로 생성되므로
  곧바로 확정해 확정본만 창과 PDF에 나간다. 확정이므로 같은 주차의 이전 확정본은 `SUPERSEDED`가
  되고 팀원에게도 공개된다. 초안 검토가 필요하면 리포트 reader의 편집·재생성 경로를 사용한다.
